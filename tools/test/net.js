...
(async () => {
  ...
  while (Date.now() - t0 < +(process.env.T || 90000)){
    ...
    if (process.env.HOSTLEAVE && !leaveDone && Date.now() - t0 > 25000){ console.log('A (anfitrión) se va'); await A.close(); leaveDone = true; pages.splice(0, 1);
      for (let i = 0; i < 60; i++){ await sleep(1000); const st = await Promise.all(pages.map(p => p.evaluate('JSON.stringify({host:NET.host,on:NET.on,gen:NET.gen,hs:NET.hostSeat,rep:NET.replaying,ab:NET.aborted,why:NET.why,app:APP})').catch(() => 'err'))); if (i % 10 === 0) console.log('migración', st.join(' | ')); if (st.every(x => x !== 'err' && JSON.parse(x).on && !JSON.parse(x).rep && (JSON.parse(x).host || JSON.parse(x).gen > 0))) { console.log('migración OK', st.join(' | ')); break; } } }
