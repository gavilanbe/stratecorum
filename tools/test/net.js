...
const STEP = `(function(){ if (APP!=='game' || !G) return 'noapp'; if (G.cur===ME && UI.mode==='idle'){ const s0 = seed; const a = aiAction(ME); seed = s0; UI.resolve(a); return 'act:'+a.type; }
  ...
  if (UI.mode==='cem'){ UI.resolve({type:'end'}); return 'cem-end'; } return (G.over ? 'over' : 'wait:'+UI.mode+':'+G.cur) + '@' + (performance.now()/1000|0); })()`;
(async () => {
  ...
  const mk = async name => { const p = await browser.newPage(); await p.setViewport({ width: 768, height: 432 }); p._errs = []; p.on('pageerror', e => p._errs.push(name + ' PAGEERROR ' + e.message)); p.on('console', m => { if (m.type() === 'error' || m.type() === 'warning') p._errs.push(name + ' ' + m.type() + ' ' + m.text()); }); return p; };
  const A = await mk('A'), B = await mk('B'), Cc = process.env.THREE ? await mk('C') : null;
  await A.goto(FILE + '?mute=1'); await A.evaluate('NET.open(); NET.create()');
  ...
  await B.goto(FILE + '?mute=1&sala=' + code);
  ...
  while (Date.now() - t0 < +(process.env.T || 90000)){
    await sleep(250);
    const rs = await Promise.all(pages.map(p => p.isClosed() ? Promise.resolve('closed') : p.evaluate(STEP)));
  ...
  console.log('ESTADOS IGUALES:', same); if (!same) console.log(hashes.join('\n')); console.log('diag', await Promise.all(pages.map(p => p.isClosed() ? 'closed' : p.evaluate('JSON.stringify({mode:UI.mode,cur:G.cur,moves:G.moves,waiters:[...NET.waiters.keys()],inbox:[...NET.inbox.entries()].map(e=>[e[0],e[1].length]),left:[...NET.left],limbo:G.limbo.length,duel:!!G.duel,pend:G.pendingMult})'))));
